const ProgressSteps = ({ step1, step2, step3 }) => {
  return (
    <div className="flex items-center justify-center space-x-4">
      ...
      <div className={`h-[2px] w-12 ${step1 && step2 ? "bg-red-600 shadow-[0_0_8px_rgba(239,68,68,0.5)]" : "bg-zinc-800"}`}></div>
      ...
      <div className={`h-[2px] w-12 ${step2 && step3 ? "bg-red-600 shadow-[0_0_8px_rgba(239,68,68,0.5)]" : "bg-zinc-800"}`}></div>

      {/* Step 3: Order */}
      <div className={`flex items-center ${step3 ? "text-red-500" : "text-zinc-600"}`}>
        <span className={`text-xs font-black uppercase tracking-widest ${step3 ? "drop-shadow-[0_0_8px_rgba(239,68,68,0.5)]" : ""}`}>03. Confirm</span>
      </div>
    </div>
  );
};
